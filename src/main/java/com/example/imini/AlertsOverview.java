...
public final class AlertsOverview {
    ...
    public static String render(Map<String, Object> stats, List<AlertSink.DedupSummary> digests,
        ...
        if (live || !recent.isEmpty() || stats.get("digest_snapshot") instanceof Map) {
            ...
            sb.append("<script>window.digestRangeActive=false;")
              ...
              .append("var qs='?limit=200'+(f?('&from='+f):'')+(t?('&to='+t):'');var n=document.getElementById('digest_range_note');")
              .append("fetch('/admin/alerts/slo-digest/history'+qs).then(function(x){if(!x.ok){return x.json().then(function(e){throw new Error((e&&e.error)||'invalid range');});}return x.json();}).then(function(hs){var h='';")
              ...
              .append("return fetch('/admin/alerts/digest-audit'+qs).then(function(x){return x.json();}).then(function(as){var h2='';as.forEach(function(x){h2+='<tr><td>'+dEsc(x.time)+'</td><td>'+dEsc(x.user||'')+'</td><td>'+dEsc(x.action||'')+'</td><td>'+dEsc((x.outcome||'').slice(0,80))+'</td></tr>';});dRows('digest_audit_body',h2);});")
              .append("}).then(function(){window.digestRangeActive=true;if(n)n.textContent=' showing '+(f||'\\u2026')+' to '+(t||'now')+' (live paused)';}).catch(function(err){window.digestRangeActive=false;if(n)n.textContent=' '+((err&&err.message)?err.message:'invalid range');});return false;}")
    ...
    private static String liveScript(int seconds) {
        return "<script>"
            ...
            + "var pb=document.getElementById('digest_posture');if(pb){pb.innerHTML=postureHtml(s.digest_snapshot);}"
            + "var u=document.getElementById('updated');if(u)u.textContent='updated '+new Date().toLocaleTimeString();}"
            + "function postureHtml(sn){if(!sn)return '';function pc(r){if(typeof r!=='number')return '\\u2014';var p=Math.round(r*1000)/10;return p+'%';}"
            + "var h='';h+='<span class=\"pill\">window '+pc(sn.window_success_ratio);if(sn.slo_target>0)h+=' / '+pc(sn.slo_target);h+='</span>';"
            + "h+='<span class=\"pill\">delivery '+pc(sn.delivery_success_ratio);if(sn.success_target>0)h+=' / '+pc(sn.success_target);h+='</span>';"
            + "if(sn.worst_route)h+='<span class=\"pill\">worst latency: '+esc(sn.worst_route)+' '+pc(sn.worst_route_ratio)+'</span>';"
            + "if(sn.worst_success_route)h+='<span class=\"pill\">worst delivery: '+esc(sn.worst_success_route)+' '+pc(sn.worst_success_route_ratio)+'</span>';"
            + "if(sn.muted)h+='<span class=\"pill muted\">muted</span>';if(sn.catchup)h+='<span class=\"pill\">catch-up pending</span>';return h;}"
