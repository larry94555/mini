...
- Grand-tour trace (edit→commit, delegation, and MCP in one session, each step cross-referenced to the test that proves it): [`docs/TRACE_TOUR.md`](docs/TRACE_TOUR.md)
- Workflow walkthrough (edit→verify→commit, hooks, MCP, delegation — with diagrams + a "how each branch is proven" map): [`docs/WORKFLOW_WALKTHROUGH.md`](docs/WORKFLOW_WALKTHROUGH.md)
...
| Skills | reusable `SKILL.md` bundles: auto-indexed, `load_skill`/`save_skill`, read-only remote repos (pinnable) via `refresh_skills`, a provenance registry (`search_skills`/`install_skill`, hash-verified), per-skill enable/disable (persisted global + per-session overrides), and member skill proposals (admin-reviewed, with a "my requests" view); bundled meta-skills `skill-builder` (research best practices on the web and capture them as a new skill) and `tool-builder` (find/install a better-fit local MCP tool, with permission); **plan-lifecycle hooks** (`skills.lifecycle`) that bind skills to planning stages (prepare/review/sub-plan/tool-select/goal-eval/post-mortem), applied automatically during plan-mode runs, observable at `GET /admin/skills/lifecycle` (configured bindings + which stages fired last run); a combined `GET /admin/capability-provisioning` view links a `tool-select` stage to the MCP server its `tool-builder` skill reloaded; proven by model-gated and node-gated end-to-end tests |
| Extensibility | MCP client (stdio + HTTP; tools, **resources & prompts**; **hot-reload** via `reload_mcp` / `POST /admin/mcp/reload` — add a server to `mcp.json` and pick it up without a restart; per-server tool counts at `GET /admin/mcp`; the live reload path is proven by a node-gated end-to-end test), research sub-agent, hooks (`PreToolUse`/`PostToolUse`/**`UserPromptSubmit`/`Stop`**), slash commands |
...
**Multi-engine web search.** The `web_search` tool queries a configurable, ordered set of **free** engines
(default DuckDuckGo + Mojeek) behind per-engine circuit breakers, then **fuses** their results with
reciprocal-rank fusion and **dedups** by canonical URL (unwrapping redirect wrappers and stripping tracking
params), returning one ranked list with each hit's **source engine** for provenance — so one engine being
rate-limited or blocked never yields empty results. The DuckDuckGo backend is hardened: it falls back from
the HTML endpoint to the stabler DDG-Lite layout, detects block/anomaly pages, and retries with browser-like
headers. All ranking/fusion/dedup runs in Java (no model tokens); results are compact by default. Configure
the engine order with `agent.web-search.engines` (e.g. `duckduckgo,mojeek`) and the result cap with
`agent.web-search.max-results`. Optional **result caching** avoids re-hitting the network (and re-spending
tokens) for repeated queries: set `agent.web-search.cache-ttl-seconds` to a positive TTL to cache fused
...
with a configurable `agent.web-search.trust-penalties` list (`host=penalty`, default neutral). Per-query
metrics — engines run/skipped, cache hit-rate, instant-answer and distillation counts — are available at the
...
**Alert notification sink.** Security-relevant audit events can page an operator instead of only being browsable. Set `alerts.enabled=true` to forward events whose action is in `alerts.actions` (default `capability_denied,spend_alert,tool_rate_limited`): each is logged at `WARN` and, if `alerts.webhook-url` is set, POSTed as a small JSON payload to that endpoint (e.g. a Slack/PagerDuty-compatible webhook) on a background thread. Forwarding is best-effort and never blocks or breaks a run; the audit write happens regardless. **Delivery is buffered and resilient:** a failed POST is retried with exponential backoff (`alerts.max-retries`, `alerts.retry-backoff-ms`), and an alert that exhausts its retries is moved to a **durable dead-letter** (the `alerts_dead_letter` SQLite table when a database is available — surviving restarts — else a bounded in-memory ring) rather than lost. Inspect dead-letters at `GET /admin/alerts/failed` (each row carries `id`/`attempts`/`last_error`/`status` retry history) and re-attempt delivery with `POST /admin/alerts/replay` (optionally `?id=<id>` for one); set `alerts.dead-letter-persistent=false` to force the in-memory ring. **Replay is crash-safe:** a row is marked `replaying` and only deleted on a confirmed 2xx — a repeated failure restores it to `failed` with updated history, and rows stuck `replaying` after a crash are reset at startup, so an alert is never lost mid-replay. If more than `alerts.queue-capacity` deliveries are in flight the newest is dropped (counted). **Structured payloads:** set `alerts.template` to a string with `{ts} {time} {user} {action} {target} {outcome}` placeholders (string fields JSON-escaped) to shape the webhook body for Slack/PagerDuty/etc.; empty uses the built-in JSON object. Dry-run a template before relying on it with `POST /admin/alerts/test` (request body = template, `?send=true` to also fire one sample) — it returns the rendered payload plus validation issues (unknown placeholders, unbalanced braces/quotes). **Per-action routing:** `alerts.routes` (entries `action|url|template` separated by `;;`) sends specific actions to their own webhook/template — e.g. `spend_alert` to a finance channel and `capability_denied` to security — with unrouted actions using the defaults. **Retention &amp; noise control:** a background reaper ages out failed dead-letters older than `alerts.dead-letter-retention-hours` (default 7 days; 0 = keep forever) so the durable store stays bounded, and `DELETE /admin/alerts/failed` (optionally `?id=`) purges on demand; set `alerts.dedup-window-seconds` to collapse repeated identical alerts (same action+target) within a window into one notification (the next alert after the window notes how many were suppressed), so a denial storm doesn't flood the channel — dedup windows are **shared across instances** via SQLite by default (`alerts.dedup-shared`, so throttling is cluster-wide, not per-process). **Escalation ladder:** `alerts.escalate-tiers` (ordered `delay|url|template` tiers separated by `;;`, where `delay` is `30s`/`15m`/`2h`/`1d`/bare-ms) re-pages an un-acknowledged dead-letter to successively higher channels as it ages — e.g. `15m|primary;;1h|secondary`. Each tier advance is an **atomic claim**, so when several instances run the reaper a tier pages exactly once. (The legacy single-tier `alerts.escalate-after-minutes`+`alerts.escalate-url` still work when no ladder is set.) **SLA-breach re-escalation:** give a tier an optional ack deadline as a 4th field (`delay|url|template|sla`) and a dead-letter that sits at that tier longer than `sla` without an ack is re-escalated to the next tier — or, at the top tier, re-paged once per SLA window — turning the ack-SLA timing into an active control (counted as `imini_alerts_sla_breaches`). Acknowledge a dead-letter with `POST /admin/alerts/ack?id=<id>` to stop escalation, or force a sweep with `POST /admin/alerts/escalate`; the tier reached (`escalation_tier`), `escalated_at`, and `acked_at` are surfaced on each dead-letter (in the JSON and the HTML viewer's **tier** column / **acked** badge), and per-tier paging is exported as `imini_alerts_escalated_tier{tier="..."}`. **Searchable backlog:** `GET /admin/alerts/failed` accepts `action`, `status`, `q` (payload substring), `offset`, and `limit` and returns `total` for paging a large backlog, and **`GET /admin/alerts.html` is a browsable HTML viewer** over the same data with filter inputs, Prev/Next paging, inline ack/replay/delete buttons, and **bulk actions** that apply to every row matching the current filter (`POST /admin/alerts/ack-all` / `POST /admin/alerts/replay-all` with the same `action`/`status`/`q` params) — so a backlog can be cleared after an outage in one click. **CSRF guard:** the viewer's state-changing actions (replay/ack/delete/bulk/escalate) require a signed token (`alerts.admin-csrf`, on by default); the page embeds it and sends it as the `X-CSRF-Token` header, so a cross-site page can't drive these endpoints. Tokens are HMAC-signed with a TTL and validated statelessly — set a shared `alerts.csrf-secret` (same value per node) so they verify **across instances** behind a load balancer (blank = random per-process, single-instance); `alerts.csrf-ttl-seconds` controls rotation. Scripts fetch a token from `GET /admin/alerts/csrf` (or pass `?csrf=`). **Dedup digests:** when a dedup window elapses with suppressed duplicates, the reaper emits one digest notification per key summarizing the count (`alerts.dedup-digest`, on by default), so a suppressed storm stays visible without flooding; the most-throttled keys are also browsable at `GET /admin/alerts/digests` and in a **"Top suppressed keys"** panel in the viewer. **Ack-SLA timing:** the time from a dead-letter being escalated to a tier until it is acknowledged is aggregated per tier (`ack_sla_by_tier` in stats) and exported as `imini_alerts_ack_latency_avg_ms{tier="..."}` / `imini_alerts_ack_latency_max_ms{tier="..."}`, so on-call responsiveness per tier is measurable. A single operator screen at **`GET /admin/alerts/overview.html`** combines the delivery counters, an **SLO summary** (latency success ratio, error budget remaining, rolling-window budget, delivery-success ratio) with a **daily success-ratio sparkline** of the rolling window (labeled with the window length, a dashed **target line**, and per-day hover tooltips like "today: 99%"; each route row also gets its own mini-sparkline trend, and the By-route table is sorted worst-trend-first so a degrading receiver surfaces at the top; per-route trend buckets are persisted (table `alert_slo_route_buckets`) so they survive a restart), per-route and per-tier breakdowns, ack-SLA latency, and top suppressed keys, and **live-auto-refreshes** by polling `GET /admin/alerts/overview.json` (tune with `?refresh=<seconds>`, `?refresh=0` to freeze). **`GET /admin/alerts/config`** returns the effective, resolved configuration (parsed tiers with SLAs, routes, dedup/retention settings, CSRF mode) with webhook URLs masked, so a misconfiguration that silently parses to nothing is easy to spot — it also returns a `warnings` array of detected misconfigurations (e.g. a ladder set while `alerts.enabled=false`, `dedup-shared` with no database, tiers that didn't parse), which are logged at startup too. **Hot-reload:** `POST /admin/alerts/reload` re-parses the alerting config (any of `actions`, `routes`, `escalate-tiers`, `slo-latency-ms`, `slo-target`; omitted pieces unchanged) into the live sink and returns the new resolved config + warnings — so a misconfiguration flagged by `GET /admin/alerts/config` can be fixed without a restart. **SLO report:** `GET /admin/alerts/slo-report` downloads the rolling-window daily good/total history (global and per route) as CSV for offline monthly reporting (`?format=json` for raw rows). The report now includes each row's effective `slo_target`/`success_target` and a `pass` flag, and accepts a date range (`?from=YYYY-MM-DD&to=YYYY-MM-DD` or `?days=N` for the last N days). **SLO digest:** set `alerts.slo-digest-interval-minutes` to periodically POST a posture summary (rolling-window budget remaining, delivery-success ratio, worst route) to `alerts.slo-digest-url` (or the default webhook) — so the SLO posture reaches people who don't open the dashboard. The message is customizable via `alerts.slo-digest-template` (placeholders like `{window_ratio}`, `{window_budget}`, `{worst_route}`, `{worst_success_route}`, `{budget_delta}`, `{since_last_minutes}`), each digest reports **deltas since the previous digest** (budget burned, delivery-success change, new dead-letters), and it names the **worst route by both latency and delivery-success** (a route can be fast but failing delivery). Send one on demand for testing with `POST /admin/alerts/slo-digest` (CSRF-guarded), or click **Send SLO digest now** on the overview page (it uses the page's CSRF token). The deltas baseline is **persisted** (table `alert_meta`) so "since last digest" survives a restart, and setting `alerts.slo-digest-via-pipeline=true` routes the digest through the normal delivery pipeline (retry + dead-letter) instead of a one-shot POST. Each posted/suppressed digest is recorded in a bounded **history** (table `alert_meta`, capped by `alerts.slo-digest-history-max`, default 50) viewable at `GET /admin/alerts/slo-digest/history` and in a "Recent SLO digests" section on the overview page. Scheduled digests can be **muted** for a known-degraded period with `POST /admin/alerts/slo-digest/mute?hours=N` (and `/unmute`), or the Mute/Unmute links on the overview page; a manual send with `?force=true` overrides the mute. The mute state is **observable**: the digest itself is prefixed `[muted]` (and exposes a `{muted}` template placeholder), Prometheus exports `imini_alerts_digest_muted` (1/0) and `imini_alerts_digest_mute_until_seconds`, and an `IminiAlertSloDigestMuted` rule surfaces it — so a silent digest isn't mistaken for a healthy one. A mute **auto-expires**: when the window elapses the mute is cleared and a resumption is logged on the next scheduled tick or manual post. Every mute/unmute/auto-expire is recorded in the **audit log** (with the acting admin user), a mute can carry a **reason** (`mute?hours=N&reason=...`, surfaced as `[muted: reason]` in the digest, a `{muted_reason}` template placeholder, and on the overview), and a request is clamped to `alerts.slo-digest-mute-max-hours` (default 72) so a mistaken long mute can't silence digests for weeks. A **reason is required** for mutes longer than `alerts.slo-digest-reason-required-hours` (default 8; the mute is rejected with HTTP 400 otherwise). The mute/unmute/auto-expire events are browsable as a **filtered audit trail** at `GET /admin/alerts/digest-audit` and in a "Digest mute audit" section on the overview page. When a mute auto-expires, the next digest is flagged as a **catch-up** (a `(catch-up after mute)` suffix and a `{catchup}` template placeholder) so the silenced window is summarized on resume. Each posted digest's history row now stores **structured metrics** (window ratio, delivery-success, budget), and the overview draws a **delivery-success trend sparkline across recent digests**. The mute audit trail is **exportable as CSV** (`GET /admin/alerts/digest-audit?format=csv`), and a catch-up send is itself recorded as an `alert_digest_catchup` audit event. The overview now charts **three trends** across recent digests — delivery-success, window SLO ratio, and window budget remaining — and the delivery-success trend is **annotated with markers** (■ a muted digest, ◆ a catch-up after mute). Both the history and audit endpoints accept a **date range** (`?from=YYYY-MM-DD&to=YYYY-MM-DD` or `?days=N`), e.g. `GET /admin/alerts/digest-audit?days=7&format=csv`. On the overview, the digest section has a **from/to date-picker** (Apply pins a ranged view and pauses live updates; Reset returns to live), the delivery-success trend's **mute/catch-up markers now persist through auto-refresh** (not just on load), and the **window-ratio trend draws the SLO target line**. The digest history endpoint now also exports **CSV** (`GET /admin/alerts/slo-digest/history?format=csv`, matching the audit CSV), the date-picker has **quick-range buttons** (24h / 7d / 30d) that set the window and apply in one click, and **History CSV / Audit CSV download links** export the current from/to range. A combined **digest report bundle** is available at `GET /admin/alerts/digest-report?from=&to=` (JSON: `{mute, history, audit}`; `?format=csv` for a single sectioned CSV) so an incident review is one download, a **Copy report link** button copies that bundle URL for the current range, and the range endpoints now **validate dates** (HTTP 400 with a clear message on a malformed `from`/`to` or `from` after `to`). The report bundle now also includes the **latest digest snapshot** (current window/delivery ratios and worst routes) — in the JSON (`snapshot`) and as a `# snapshot` CSV section — the overview has a **Download report bundle** link (the digest-report CSV for the current range), and a bad range entered in the picker now **shows the validation error inline** (the view isn't pinned) instead of failing silently. The overview digest section now opens with a compact **current-posture row** (window/delivery ratios vs targets, worst routes, mute/catch-up) rendered from the live snapshot; the scheduled webhook digest payload carries the snapshot as **structured fields** (`{"text":"...","digest":{...}}`, keeping `text` for back-compat); and the report link/download offers a **JSON or CSV** choice. The current-posture row now **live-refreshes** on each overview poll (rebuilt from `overview.json`, no longer stale until reload); the structured webhook payload can be turned off with `alerts.slo-digest-structured=false` (default on) for receivers that only want `{"text":"..."}`; and the posture is exported as Prometheus gauges (`imini_alerts_digest_window_ratio`, `imini_alerts_digest_delivery_ratio`, `imini_alerts_digest_worst_route_ratio`, `imini_alerts_digest_worst_success_route_ratio`) so it's alertable, not just visible. Set `alerts.config-override-file` to a writable path to make a reload **durable** — the reloaded config is written there and re-applied at startup (blank = in-memory only, lost on restart). **Self-test:** `POST /admin/alerts/selftest` pushes a synthetic alert through routing/dedup resolution and reports where it lands; with `?send=true` it does a live probe POST and reports the receiver's HTTP status and round-trip latency, so wiring can be verified without a real incident. **Delivery latency:** webhook POST round-trip time is recorded as a Prometheus histogram (`imini_alerts_delivery_latency_ms_bucket`/`_sum`/`_count`), so a slow receiver is visible before it starts failing, and **per-route average latency** is exported as `imini_alerts_route_latency_avg_ms{route="..."}` so you can see *which* receiver is slow. **Latency SLO:** define an objective with `alerts.slo-latency-ms` (default 1s) and `alerts.slo-target` (default 0.99) — the observed success ratio and error-budget **burn rate** are exposed in stats and as `imini_alerts_slo_success_ratio`/`imini_alerts_slo_burn_rate`, with monotonic `imini_alerts_slo_good_total`/`imini_alerts_slo_total_total` counters so the [`ops/`](ops/README.md) multi-window burn-rate rules reference the app's own good/total and `imini_alerts_slo_target` — the objective lives only in config, with no hard-coded latency bucket to drift. **Per-route SLO:** the same success-ratio/burn is computed per route (`imini_alerts_route_slo_success_ratio`/`_burn_rate`/`_good_total`/`_total_total{route="..."}`), so a single degraded receiver pages even when the global SLO looks healthy — and each route can carry its **own objective** via two optional `alerts.routes` fields (`action|url|template|latency|target`, e.g. `spend_alert|https://hook||500|0.999`), inheriting the global objective when omitted. **Error budget remaining:** alongside burn rate, the SLO reports how much of the budget is left over the observed window (`imini_alerts_slo_budget_remaining`, negative = exhausted; also per route), the runway teams report on rather than instantaneous burn. **Rolling window:** because the cumulative figure never resets, a rolling-window budget over the last `alerts.slo-window-days` (default 30) is tracked separately and exposed as `imini_alerts_slo_window_budget_remaining`/`_success_ratio`/`_burn_rate` — the "how much runway is left this month" number. When a database is available these daily buckets are **persisted** (table `alert_slo_buckets`, flushed on the reaper tick and at shutdown) so the rolling window survives a restart; rows that age out of the window are pruned on each flush so the table stays bounded. **Delivery-success SLO:** separate from latency, an objective that at least `alerts.success-target` of finalized deliveries actually land (2xx) rather than dead-letter is exposed as `imini_alerts_success_slo_*` and per route as `imini_alerts_route_success_ratio{route=". Each route can also set its own success target via a 6th `alerts.routes` field (`action|url|template|latency|target|success-target`)..."}`, so a receiver that is fast but returning 5xx pages too. **Scheduled self-test:** set `alerts.selftest-interval-minutes` to periodically push a synthetic alert through the pipeline and export `imini_alerts_selftest_ok` (1/0) so wiring breakage is caught proactively; by default it only checks resolution, `alerts.selftest-send=true` adds a live probe POST (point `alerts.selftest-action` at a route wired to a health endpoint). Recent runs are kept and surfaced at `GET /admin/alerts/selftest`, with **flap detection** (`alerts.selftest-flap-threshold`) exporting `imini_alerts_selftest_flapping` so an intermittent failure is distinguished from a steady one. The [`ops/`](ops/README.md) bundle ships a Grafana dashboard, Prometheus alert rules (escalation, SLA-breach, ack-latency, suppression-storm), and a response runbook for these signals. Delivery counters (`sent`/`failed`/`retried`/`dead_lettered`/`dropped`/`replayed`/`suppressed`/`escalated`/`digested`/`sla_breaches`), **per-route counters** (`imini_alerts_route_sent`/`_failed`/`_dead_lettered`/`_suppressed`{route="..."}), and per-action audit counts are exported to Prometheus — a ready-made Grafana dashboard and Prometheus alert rules live in [`ops/`](ops/README.md). Off by default (no forwarding).
...
| `POST /admin/alerts/replay` | Re-attempt dead-lettered alert deliveries (`?id=` for one) (admin) |
| `POST /admin/alerts/test` | Dry-run/preview an alert template against a sample (`?send=true` to fire) (admin) |
| `DELETE /admin/alerts/failed` | Purge dead-lettered alerts, all or `?id=` (admin) |
| `POST /admin/alerts/ack` | Acknowledge a dead-letter so it stops escalating (`?id=`) (admin) |
| `POST /admin/alerts/escalate` | Force an escalation sweep of un-acked dead-letters (admin) |
| `GET /admin/alerts.html` | Browsable dead-letter viewer (filter, page, ack/replay/delete) (admin) |
| `POST /admin/alerts/ack-all` | Bulk-ack all dead-letters matching `action`/`status`/`q` (admin) |
| `POST /admin/alerts/replay-all` | Bulk-replay all dead-letters matching `action`/`status`/`q` (admin) |
| `GET /admin/alerts/csrf` | Fetch the per-process CSRF token for the viewer's actions (admin) |
| `GET /admin/alerts/digests` | Top currently-throttled dedup keys, most suppressed first (admin) |
| `GET /admin/alerts/overview.html` | One-screen alerting overview: counters, routes, tiers, SLA (admin) |
| `GET /admin/alerts/overview.json` | JSON backing the overview page's live auto-refresh (admin) |
| `GET /admin/alerts/config` | Effective resolved alerting config, webhooks masked (admin) |
| `POST /admin/alerts/reload` | Hot-reload alerting config (routes/tiers/SLO) into the live sink (admin) |
| `GET /admin/alerts/slo-report` | Download the rolling-window SLO history as CSV (`?format=json` for rows) (admin) |
| `POST /admin/alerts/selftest` | Synthetic end-to-end pipeline self-test; `?send=true` probes the webhook (admin) |
| `GET /admin/alerts/selftest` | Recent self-test history + flap assessment (admin) |
...
session that approved them — one run cannot widen another's access — while the default root is shared.
Grants are **durable**: they persist to the database and reload on restart, with an optional
