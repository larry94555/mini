...
| `scripts/check-docs.sh` | Fail if README/docs reference a test class, `.java` file, TESTING case, or relative Markdown link target that doesn't exist (`WARN_ONLY=1` to report-only). Run it with `./run.sh check-docs`; also runs in CI |
