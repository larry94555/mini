...
| Extensibility | MCP client (stdio + HTTP; tools, **resources & prompts**; **hot-reload** via `reload_mcp` / `POST /admin/mcp/reload` — add a server to `mcp.json` and pick it up without a restart), research sub-agent, hooks (`PreToolUse`/`PostToolUse`/**`UserPromptSubmit`/`Stop`**), slash commands |
