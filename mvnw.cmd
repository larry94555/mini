...
for /f "tokens=2 delims==" %%s in ('findstr /b "distributionSha256Sum=" "%BASEDIR%.mvn\wrapper\maven-wrapper.properties" 2^>nul') do set "SHA=%%s"
...
if not exist "%MVN_BIN%" (
  ...
  powershell -NoProfile -ExecutionPolicy Bypass -File "%BASEDIR%scripts\get-maven.ps1" -Version %VER% -Dest "%BASEDIR%.maven" -Sha256 "%SHA%"
