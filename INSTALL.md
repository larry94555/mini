...
| `ask.sh` answer ends with `--- Coding report: ...` instead of a plain reply | The coding-report post-processor is on. In `src/main/resources/application.properties` set `agent.coding-report=false`, `agent.coding-report.enforce=false`, and `agent.verify-edits=false`, then restart `./run.sh`. These are off by default in recent versions; you may have an older checkout. |
| `ask.sh` answer is `[harness stopped this response: time cap (90s)]` | The model is running on CPU-only hardware and needs more than 90s to finish. In `src/main/resources/application.properties` set `agent.stream-max-seconds=600` and restart `./run.sh`. |
| Config changes to `application.properties` seem ignored (still see old behavior, e.g. `time cap (90s)` or a coding report after you set them off) | The running server is using a stale build. Stop `./run.sh` (Ctrl+C), then do a clean rebuild: `rm -rf target && ./run.sh`. Spring Boot copies `application.properties` into `target/` at build time, so a stale `target/` keeps old values. (Recent versions also set the same values as code defaults, so a clean rebuild is enough.) |
| First `ask.sh` after startup is very slow on CPU-only machines | The full prompt (system prompt + tools) is several thousand tokens; on a CPU at ~40 tokens/sec, processing it takes ~2 minutes **before the first word is generated**. This is why the time cap defaults to 600s. The good news: llama.cpp caches the prompt prefix, so the *second* and later questions are much faster. Be patient on the first one. |
| `[llama] started on port 8081` appears in run.sh but `[llama] ready.` never does — ask.sh has no response (WSL/Linux) | Java's HTTP client is resolving `localhost` to `::1` (IPv6) but llama-server only listens on IPv4. In `application.properties` set `llama.client-host=127.0.0.1` (the default in recent versions). |
